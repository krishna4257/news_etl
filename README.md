...
## 📂 Project Structure
```
news_etl/ │── pycache/ # Python cache files (ignore) │── venv/ # Virtual environment (ignore) │── .gitignore # Git ignore file │── docker-compose.yaml # Docker setup for Airflow │── extract.py # Extracts news data from NewsAPI │── load.py # Loads structured data into NeonDB │── transform.py # Transforms JSON data into structured format │── automate.py # DAG definition for Airflow orchestration │── requirements.txt # Required Python libraries │── README.md # Project documentation
```
